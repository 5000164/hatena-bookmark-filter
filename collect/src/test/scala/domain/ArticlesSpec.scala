...
import domain.Articles.fetchDeliveredArticles
...
class ArticlesSpec extends FeatureSpec {
  feature("記事を抽出できる") {
    scenario("XML から記事を抽出する") {
      assert(
        fetchDeliveredArticles(feedUrl = "feedUrl", fetchContent = _ => Some(Source.fromResource("sample.rss").getLines.mkString)) === Seq(
    ...
    scenario("記事が取得できなかった場合は対象なしにする") {
      assert(
        fetchDeliveredArticles(feedUrl = "feedUrl", fetchContent = _ => None) === Seq()
      )
    }
  }
}
